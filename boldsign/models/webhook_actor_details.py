...
class WebhookActorDetails(BaseModel):
    """
    WebhookActorDetails
    """ # noqa: E501
    user_type: StrictStr = Field(alias="userType")
    id: Annotated[str, Field(min_length=1, strict=True)]
    __properties: ClassVar[List[str]] = ["userType", "id"]

    @field_validator('user_type')
    def user_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Sender', 'Signer', 'Admin', 'Creator', 'SenderIdentity', 'Cc']):
            raise ValueError("must be one of enum values ('Sender', 'Signer', 'Admin', 'Creator', 'SenderIdentity', 'Cc')")
        return value
